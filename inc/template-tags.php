<?php
/**
 * Custom template tags for Full Stack Entrepreneurship
 *
...
if ( ! function_exists( 'ign_posted_by' ) ) {
	function ign_posted_by() {
		...
		$author_name        = sprintf( __( '%s by %s', 'full-stack-entrepreneurship' ), '<a href="' . $author_link . '" class="author-name"><span class="byline">', '</span>' . get_the_author() . '</a>' );
	...
	function ign_get_terms( $taxonomy = 'category', $get_all = false ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) :
			...
			if ( ! $get_all ) {
				...
				$term_links_output = join( '<span class="delim">' . __( ', ', 'full-stack-entrepreneurship' ) . '</span>', $term_links );
			}
...
function ign_edit_link( $id = null, $class = '', $text = 'Edit' ) {
	...
	edit_post_link(
		...
		/* translators: %s: Name of current post */
			__( '%s<span class="screen-reader-text"> "%s"</span>', 'full-stack-entrepreneurship' ),
			$text, get_the_title()
		),
